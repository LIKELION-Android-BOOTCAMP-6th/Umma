...
class BuildSessionCompressionPayloadUseCase @Inject constructor() {
    ...
    operator fun invoke(
        ...
        return runCatching {
            // topicSummaries 는 "무엇을 교정했는지"를 남기는 압축 요약이다.
            // AI Chat 원문 전체를 보존하지 않아도 이후 맥락을 복원할 수 있게 before -> after 형태로 만든다.
            val topicSummaries = selectedSuggestions
            ...
            val recentTopics = extractRecentTopics(
