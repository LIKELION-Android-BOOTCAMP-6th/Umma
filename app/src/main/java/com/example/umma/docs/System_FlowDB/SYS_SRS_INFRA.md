...
| 3. Review 스케줄 정책 | SM-2 기반 4단계 평가, ReviewSchedulePolicy, local first 완료 파이프라인, summary 갱신 기준 정의 | 평가 결과가 다음 복습 시점과 요약값에 일관되게 반영됨 | 성공: 스케줄/요약 갱신 가능 / 실패: next_review_at, pending sync 기준 불명확 | SRI-003 |
...
- [SRI-001 Flashcard 반복학습 모델 계약](./SYS_SRS_INFRA/SRI-001_Flashcard_Model.md)
- [SRI-002 Flashcard Repository 조회/저장 계약](./SYS_SRS_INFRA/SRI-002_Repository_Contract.md)
- [SRI-003 Review 스케줄 정책 및 완료 파이프라인](./SYS_SRS_INFRA/SRI-003_Schedule_Policy.md)

### User Flow 구현 이슈

- [SRS-001 반복학습 진입 및 언어 컨텍스트](../User_FlowDB/FLOW_SRS/SRS-001_Entry_Context.md)
- [SRS-002 복습 카드 덱 로드](../User_FlowDB/FLOW_SRS/SRS-002_Deck_Load.md)
- [SRS-003 카드 앞/뒤 표시 및 뒤집기](../User_FlowDB/FLOW_SRS/SRS-003_Card_Flip.md)
- [SRS-004 발음 재생](../User_FlowDB/FLOW_SRS/SRS-004_Pronunciation_Playback.md)
- [SRS-005 복습 평가 및 스케줄 반영](../User_FlowDB/FLOW_SRS/SRS-005_Grading_and_Schedule.md)
- [SRS-006 완료, 복귀, 동기화](../User_FlowDB/FLOW_SRS/SRS-006_Completion_and_Return.md)
...
→ SrsStudy 화면 진입
→ FlashcardRepository에서 due deck 조회
→ 카드 앞면 / 뒷면 표시
→ 사용자가 평가 버튼 선택
→ ReviewSchedulePolicy로 next_review_at 계산
...
실제 복습 카드 순서는 Flashcard 원본의 `next_review_at` 기준으로 계산한다.
...
- `next_review_at <= now`인 카드가 복습 대상이다.
...
| `ReviewScheduleResult` | interval, ease factor, next review time 계산 결과 |
