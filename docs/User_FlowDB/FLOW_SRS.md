...
- `SYS-SRS-INFRA`의 Flashcard 조회/복습 결과 갱신 계약 참조 가능 상태
...
- SrsStudy route로 진입했을 때 현재 선택 언어 기준 학습 컨텍스트를 초기화할 수 있다.
...
- review 결과 저장 후 `FlashcardSummary`와 `DashSummary`의 due 수치가 최신 상태로 반영된다.
...
| --- | --- | --- | --- | --- | --- | --- |
| 반복학습 진입 | SrsStudy route 진입 | selectedLearningLanguage와 학습 컨텍스트 확인 | SrsStudy 화면 진입 | 라우트/컨텍스트 없음 | Loading / Ready / Error | SRS-001 |
...
| 완료 및 복귀 | review 결과 저장 완료 또는 뒤로가기 | 다음 카드 이동, 덱 완료, Summary 최신 상태 관찰 | Dashboard 복귀 또는 완료 상태 | 동기화 지연 / 복귀 실패 | Completing / Done / PendingSync | SRS-006 |
