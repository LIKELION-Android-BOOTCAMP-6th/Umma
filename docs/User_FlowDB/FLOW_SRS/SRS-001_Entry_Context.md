...
사용자는 Dashboard의 Flashcard 학습 카드나 직접 경로를 통해 SrsStudy 화면에 진입하고,
...
- [ ] Dashboard 카드 또는 직접 경로로 SrsStudy 화면에 진입할 수 있다.
...
- 중복 Navigation 방지
- direct entry와 Dashboard entry의 공통 초기화
...
- Dashboard의 Flashcard 학습 카드
- 직접 경로 진입

두 경로는 같은 초기화 규칙을 공유한다.
...
- Dashboard 카드 수와 실제 deck 수가 잠시 다른 경우
