...
Correction 쪽에서 Room DAO 준비 전까지 사용하는 in-memory local 저장은 임시 구현이며,
SRI-002에서는 같은 Flashcard 원본 계약을 Room Entity / DAO 기반 저장소로 이어받는 것을 목표로 한다.

---

## 완료 기준(AC) (Acceptance Criteria)

- [ ] `FlashcardRepository`가 due card 조회와 review 결과 schedule 갱신 요청을 함께 다루는 계약으로 준비된다.
- [ ] Correction의 임시 in-memory Flashcard local 저장 계약을 Room Entity / DAO 기반 저장소로 승계한다.
...
- Correction의 in-memory local 저장은 Room DAO 준비 전까지 계약을 검증하기 위한 임시 구현이다.
- SRI-002 작업에서는 같은 필드 의미를 가진 Room Entity / DAO를 준비해 SRS와 Correction이 같은 Flashcard 원본을 바라보게 한다.
...
- Room DAO가 준비되면 `CorrectionRepository.saveFlashcards(...)`의 local 저장도 같은 Flashcard DAO를 사용하도록 교체한다.
