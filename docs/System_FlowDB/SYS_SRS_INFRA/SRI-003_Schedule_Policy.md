...
- [ ] MVP에서는 SM-2 전체 알고리즘을 1:1로 복제하지 않고, `interval`, `ease_factor`, `next_review_at` 중심의 단순화 정책을 사용한다.
- [ ] `ReviewSchedulePolicy`가 `ReviewDecision`을 받아 `ReviewScheduleResult`를 계산한다.
- [ ] `interval`, `ease_factor`, `next_review_at` 갱신 기준이 확정된다.
...
- SM-2 기반 4단계 평가 정책
- `interval`, `ease_factor`, `next_review_at` 계산 기준
...
이 정책은 SM-2의 핵심 값인 `interval`, `ease_factor`, `next_review_at`을 사용한다.
...
`ease_factor`, `interval`, `next_review_at`은 이 정책을 반영해 갱신한다.  
...
- `next_review_at`이 지나지 않은 카드는 due deck에 포함하지 않는다.
...
- [SRS-005_Grading_and_Schedule.md](../../User_FlowDB/FLOW_SRS/SRS-005_Grading_and_Schedule.md)
- [SRS-006_Completion_and_Return.md](../../User_FlowDB/FLOW_SRS/SRS-006_Completion_and_Return.md)
