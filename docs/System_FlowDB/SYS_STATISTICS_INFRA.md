...
| --- | --- | --- | --- |
| 기존 문서 | `SYS_LEARNING_STATE_INFRA` / `LS-006` | Correction 완료 파이프라인에서 External Metrics 재계산과 local `LangState` 저장 성공 이후 Statistics 쪽 기록 UseCase를 호출할 연결 지점을 명시한다. LS-006 계산/저장 파이프라인 자체는 수정 범위가 아니다. | STI-002 |
...
Statistics history 기록을 안전하게 연결하려면 LS 쪽 선행 계약에서 아래 항목이 확인되어야 한다.
