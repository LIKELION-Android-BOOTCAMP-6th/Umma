...
`SYS-CORRECTION-INFRA`는 새 Flashcard 최초 저장에 필요한 Room Entity / DAO / local source를 선행 제공한다.
`SYS-SRS-INFRA`의 Flashcard 저장소 작업에서는 이 Room 원본을 조회하고 review 결과로 schedule을 갱신하는 repository 계약을 준비한다.
조회/갱신의 실제 repository 구현은 SRS 책임이지만, 사용할 local source 통로는 `SYS-CORRECTION-INFRA`가 제공한 due 조회와 review schedule 갱신 계약을 재사용한다.

- Flashcard 복습 대상 조회 기준
- SM-2 기반 4단계 평가 버튼과 SRS 스케줄 갱신 정책
- FlashcardRepository / UseCase 경계
- Android TextToSpeech 기반 발음 재생 입력 계약
...
| 3. Review 스케줄 정책 | SM-2 기반 4단계 평가, ReviewSchedulePolicy, local first schedule 갱신, summary 연동 경계 정의 | 평가 결과가 다음 복습 시점과 Dashboard 요약에 일관되게 반영됨 | 성공: 스케줄 및 summary 갱신 가능 / 실패: nextReviewAt 또는 summary 연동 경계 불명확 | SRI-003 |
...
- [SRI-001 Flashcard 반복학습 모델 계약](https://github.com/LIKELION-Android-BOOTCAMP-6th/Umma/issues/82)
- [SRI-002 Flashcard Repository 조회/복습 결과 갱신 계약](https://github.com/LIKELION-Android-BOOTCAMP-6th/Umma/issues/83)
- [SRI-003 Review 스케줄 정책 및 Summary 연동 경계](https://github.com/LIKELION-Android-BOOTCAMP-6th/Umma/issues/84)
...
- `FlashcardSummary.dueFlashcards` / `DashSummary.dueFlashcards` 갱신 기준

`SYS-SRS-INFRA`의 갱신 계약은 Flashcard 최초 생성 저장이 아니라, 복습 평가 이후 기존 Flashcard의 schedule/review 상태를 갱신하는 계약이다.
Correction에서 선택한 교정 결과를 새 Flashcard로 만드는 Room DAO local first 저장은 `SYS-CORRECTION-INFRA`에서 다룬다.
SRS는 Correction이 저장한 Flashcard Room 원본을 조회하고, review 결과를 같은 원본의 schedule 상태에 반영한다.
...
| `data` | Correction이 만든 Flashcard Room 원본 조회, review schedule local first 갱신, Firestore sync, DTO/Entity 변환, due 카드 조회 |
...
Flashcard Room 원본은 `SYS-CORRECTION-INFRA`가 제공하며, SRS는 이 원본을 조회/갱신하는 repository 계약을 별도로 둔다.
SRS는 같은 원본을 읽기 위해 별도 중복 Entity를 만들지 않고, Correction이 제공한 due 조회와 review schedule 갱신 통로를 감싼다.
...
→ 같은 Flashcard 원본에서 due/saved count 재계산
...
- `FlashcardSummary.dueFlashcards`와 `DashSummary.dueFlashcards`는 SRS review 완료 흐름에서 LS의 `ApplyFlashcardSummaryUpdateUseCase`를 통해 함께 반영한다.
- Flashcard review schedule local 갱신과 Summary 업데이트는 서로 다른 저장소 계약을 사용하지만, `ApplyReviewDecisionUseCase`가 같은 local completion 흐름으로 조율한다.
- Summary 갱신 실패 시 이전 schedule 값으로 보상 갱신하고, 완료 실패로 반환한다.
...
SRS는 카드 조회와 복습 결과 schedule 갱신이 모두 포함되므로 mock/real 교체 가능 구조가 필요하다.
