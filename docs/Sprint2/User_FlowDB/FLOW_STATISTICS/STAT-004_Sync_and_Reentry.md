...
- `background refresh`는 Firestore의 최신 history를 가져와 local Room을 보정하는 `remote → local` 흐름이다.
- `pending sync`는 local에 먼저 저장된 history가 Firestore에 아직 반영되지 않은 `local → remote` 대기 상태다.
- Statistics 화면은 refresh 결과를 UI에 직접 주입하지 않고, local Room에 반영된 결과를 observe 해서 다시 그린다.
- pending sync history는 local에 있으면 chart source로 사용할 수 있다.
...
remote refresh는 화면 state를 직접 바꾸는 것이 아니라 local cache 보정 역할만 한다.
...
- 현재 사용자 또는 현재 언어와 다른 background refresh 결과는 화면 state에 반영하지 않는다.
...
- [STAT-003_Metric_Line_Chart.md](https://github.com/LIKELION-Android-BOOTCAMP-6th/Umma/blob/develop/docs/Sprint2/User_FlowDB/FLOW_STATISTICS/STAT-003_Metric_Line_Chart.md)
