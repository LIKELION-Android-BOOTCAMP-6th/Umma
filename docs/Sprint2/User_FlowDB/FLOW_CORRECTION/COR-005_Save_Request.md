...
- [ ] 저장 대상이 0개이면 완료 파이프라인을 호출하지 않는다. 단, `COR-TUNE-007` 품질 필터 이후 저장 가능한 카드가 0개가 된 경우는 저장 실패가 아닌 no-op 완료 예외로 `CompleteCorrectionUseCase`가 처리한다.
...
- 선택된 카드만 저장 요청에 포함한다.
- 저장 대상이 0개이면 UseCase를 호출하지 않는다. 단, 선택 항목은 있었지만 `COR-TUNE-007` 품질 필터로 모두 제외된 빈 저장 요청은 완료 세션을 닫기 위해 `CompleteCorrectionUseCase`의 no-op 완료 경로로 넘긴다.
...
- 저장 요청에는 사용자별 Room 저장을 위해 현재 사용자 ID가 포함되어야 한다.
- 저장 버튼 클릭 후 요청 준비 중에는 중복 클릭을 막는다.
- 저장 대상이 0개인 경우 UseCase를 호출하지 않는다. 품질 필터로 사후 0개가 된 요청은 저장소 호출 없이 완료 성공으로 정리되는 예외 케이스다.
