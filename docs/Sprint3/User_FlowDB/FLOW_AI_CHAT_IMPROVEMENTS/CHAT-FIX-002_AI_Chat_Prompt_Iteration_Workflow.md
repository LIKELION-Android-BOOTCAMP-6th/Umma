...
- [ ] 같은 band의 대화 운영 원칙은 selected language가 달라도 동일하게 유지하고, 언어별 차이는 예시 문장과 짧은 학습언어 조각에만 둔다.
...
- 프롬프트 개선 우선순위와 수정 단위
- selected language가 달라도 같은 band 리듬을 유지하는 기준
- 영어/일본어 외 지원 언어에도 가능한 범위에서 검증된 언어쌍별 전용 예시를 두고, 준비되지 않은 언어쌍에 영어 예시를 fallback으로 넣지 않는 기준
- 개선 후 검증 기준
...
- 기존 문장의 압축/수정으로 해결 가능한 문제는 새 문장 추가보다 먼저 처리한다.
- 새 문장이 꼭 필요하면 짧은 1문장 단위로 제한해 추가하고, revision별 효과를 본 뒤 유지/제거를 결정한다.
- 한 번에 하나의 주된 원인만 수정한다.
- prompt가 비대해지면 문장 추가보다 중복 제거를 우선한다.
- 같은 band에서는 언어별로 다른 대화 철학을 만들지 않는다. 언어별 차이는 `style_reference` 예시와 자연스러운 학습언어 조각에만 둔다.
...
- 초반은 잘 작동하지만 긴 대화에서 상담/작업 보조/한국어 단독 응답으로 흐려지는 문제는 이번 커밋에서 해결한 범위로 보지 않는다.
- 이 문제는 첫 응답/언어별 예시 문제와 분리해 다음 신고 세션 분석에서 별도 원인으로 다룬다.
