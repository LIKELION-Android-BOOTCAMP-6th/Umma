...
- [ ] 사용자와 AI의 final 자막은 메신저 대화처럼 순서대로 아래에 추가된다.
...
- 자막은 SessionMemory 전체 로그가 아니라 현재 Chat 화면의 표시용 대화 흐름이다.
- 새 자막은 메신저 대화처럼 아래쪽에 추가된다.
...
- 사용자 final 자막과 AI final 자막이 발화 순서대로 아래에 추가된다.
