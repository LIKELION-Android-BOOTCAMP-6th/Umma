...
- **빈 결과 완료 정책**: 품질 필터로 저장 가능한 카드가 0개가 되면 `CompleteCorrectionUseCase`는 Flashcard 저장소를 호출하지 않는 no-op 완료 성공으로 처리한다. 이때 LangState 점수/교정 근거 분석은 돌리지 않고 `correctionAvailable=false` 신호만 반영해 같은 교정 세션이 다시 노출되지 않게 한다.
...
3. **빈 결과/부분 제외 흐름 정의** (`CompleteCorrectionUseCase` 회귀 확인)
   ...
   - 전부 제외된 경우에는 저장·LangState 분석·statistics·compression을 건너뛰고 correctionAvailable 신호만 false로 닫는다.
...
- 전부 제외된 no-op 완료는 학습 점수에 교정 0개 또는 선택 suggestion 전체를 반영하지 않는다. 저장 가치가 없는 카드가 LangState metric evidence로 남지 않게 하기 위함이다.
