...
- [ ] 신고 데이터의 보관 목적, 검토 가능성, 90일 보관, 삭제 요청 처리 정책이 정리된다.
...
- 운영용 신고 데이터의 기본 보관 기간은 신고 접수일 기준 90일로 둔다.
...
- AI 안전 신고 문서는 uid를 포함하지 않는 결정론적 reportId(`report_{sessionId}_{reportedTurnId}` 형식, 서버 계산)를 사용하고, 원문 대화는 분석 목적상 유지하되 사용자 식별자는 제거하거나 가명화한다.
