...
- [ ] 운영용 AI 신고 데이터는 90일 보관 후 자동 정리될 수 있도록 만료 기준을 가진다.
...
- **운영용 AI 신고는 90일 보관 후 정리한다.**
  ...
  - 보관 기간은 90일로 두고, `expiresAt` 또는 동등한 만료 기준으로 자동 정리 가능하게 만든다.
...
- `ai_content_reports`에는 90일 자동 정리를 위한 `expiresAt` 또는 동등한 만료 기준을 추가한다.
...
- 필요하면 `ai_content_reports`의 사용자 직접 식별 필드는 별도 운영 정책으로 축소할 수 있으나, 이번 작업의 기본 결정은 90일 제한 보관이다.
